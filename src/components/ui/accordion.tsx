...
const VendorMap: React.FC<VendorMapProps> = ({ 
  ...
  const vendors = useMemo(() => {
    return filteredVendors || vendorService.getAll().filter((v: Vendor) => v.isActive);
  }, [filteredVendors]);
  ...
  const mapCenter: [number, number] = useMemo(() => {
    ...
    if (vendors.length > 0) {
      const avgLat = vendors.reduce((sum: number, v: Vendor) => sum + (v.location?.latitude || 0), 0) / vendors.length;
      const avgLng = vendors.reduce((sum: number, v: Vendor) => sum + (v.location?.longitude || 0), 0) / vendors.length;
